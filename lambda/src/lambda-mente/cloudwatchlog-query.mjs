 ...
 *   でファイルが作成されます。
 *
下記の前提と要件で Nodejs を使い aws lambda のソースを作成
前提
...
要件
 ...
 5. ログクループ名を配列に保持
 6. 5 の件数が 50 件に達したら、StartQueryCommand を実行し、非同期実行の配列に保持
 7. すべてのロググループの判定が完了し、5 の配列に端数がある場合、残り分について 6 を実行
 8. 6 の配列を非同期に GetQueryResultsCommand 実行状態を取得し、ログ取得が完了していた場合、結果を今日の日付のファイルを s3 に書き出す
 *
 */
import {
