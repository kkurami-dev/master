...
const requestHandler_low = new FetchHttpHandler({
  connectionTimeout: 50,
  socketTimeout: 200,
}); //
const aws_config = {
 ...
 */
// let cip_client;
export async function login(username, password, clientId, cb){
  Auth.signIn(username, password)
    .then(user => {
      ...
      if (user.challengeName === 'NEW_PASSWORD_REQUIRED') {
        ...
        console.log( "login : other situations" );
        if(cb) cb();
      }
    }).catch(e => {
      return e.message;
...
//     ClientId: clientId,
//   });
//   console.log('login 3', command);
//   const ret = await cip_client.send(command);
//   console.log('login 4', ret);
//   console.log('login 4 end', ret.AuthenticationResult);
  ...
  in_config=null)
{
  ...
  return {
    S3GetObject: (params, cb=undefined) => s3GetObjectMain(params, cb),
    S3PutObject: (params, cb=undefined) => s3PutObjectMain(params, cb),
    S3GetSignedUrl: (params, cb=undefined) => s3GetSignedUrlMain(params, cb),
    S3PutSignedUrl: (params, cb=undefined) => s3PutSignedUrlMain(params, cb),
  };
}
   ...
   )
{
  ...
  const comSend = async (command, cb=undefined, retry=true) => {
    ...
    do{
      ...
      return data;
    } while( ++loop_count < ROOP_MAX && retry);
    //return error;
  };
